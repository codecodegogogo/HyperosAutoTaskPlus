...
/**
 * 与自动任务引擎（b2.j / h6.m 单例）之间的桥：把「这些条件项可能变了，请重新判定」交给引擎。
 *
 * 引擎入口在 12.x 是 public void K(ConcurrentHashMap<String, TaskItem>)，13.5/13.6 是
 * public void O(ConcurrentHashMap<String, TaskItem>)；内部再投递到引擎自己的 auto_task 线程，
 * 逐个调 m()，满足就执行/恢复任务——和它处理前台切换、WLAN 变化的路径完全相同。
 * 各运行时（进程跟踪、屏幕状态……）共用这一份，引擎实例由 hook 侧在构造函数完成后交进来。
 */
final class EngineBridge {
    ...
    private static Method findNotifyMethod(Object engine) throws NoSuchMethodException {
        ...
        // K()/O() 是 public 的对外入口；名字随版本混淆可能变化，按签名兜底
        try {
        ...
        throw new NoSuchMethodException("引擎通知入口 K/O(ConcurrentHashMap) 未找到");
    }
}
